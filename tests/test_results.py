...
def test_repr(engine: Replus) -> None:
    match = found(engine.search("Today is january 1st 1970"))
...
def test_nested_group_query(engine: Replus) -> None:
    match = found(engine.search("Today is january 1st 1970"))
    date_group = found(match.group("date"))
    assert found(date_group.group("day")).value == "1"
    assert [g.value for g in date_group.groups("year")] == ["1970"]
    assert found(date_group.group("day")).groups() == []
...
def test_group_serialize_shape(engine: Replus) -> None:
    year = found(found(engine.search("Today is january 1st 1970")).group("year"))
...
def test_reps_of_single_group_is_empty(engine: Replus) -> None:
    match = found(engine.search("Today is january 1st 1970"))
    assert found(match.group("year")).reps() == []


def test_reps_navigate_their_own_span(engine: Replus) -> None:
    match = found(engine.search("foobar 34 of 1997 15 of 1988 45 of 1975"))
    reps = found(match.group("numyear")).reps()
    assert [rep.value for rep in reps] == ["34 of 1997", "15 of 1988", "45 of 1975"]
    assert found(reps[2].group("num")).value == "45"
    assert found(reps[2].group("fooyear")).value == "1975"


def test_explicit_rep_index(engine: Replus) -> None:
    match = found(engine.search("foobar 34 of 1997 15 of 1988 45 of 1975"))
    numyear = found(match.group("numyear"))
...
def test_group_captured_outside_match_span_is_excluded() -> None:
    ...
    match = found(Replus(templates).search("alphax"))
...
def test_group_type() -> None:
    ...
    match = found(Replus(templates).search("42"))
